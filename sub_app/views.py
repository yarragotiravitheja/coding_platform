...
def run_code(request):
    if request.method == 'POST':
        ...
        fileloc = path.join(BASE, "media", filename)
        ...
        try:
            if lang == "PYTHON":
                ...
                if compile_output.returncode != 0:
                    raise Exception(compile_output.stderr)
                output = subprocess.run(["./a.out"], input=stdin.encode('utf-8'), timeout=5, capture_output=True,
                                        text=True)
            elif lang == "C":
                compile_output = subprocess.run(["gcc", fileloc, "-o", "a.out"], timeout=5, capture_output=True,
                                                text=True)
                if compile_output.returncode != 0:
                    raise Exception(compile_output.stderr)
                output = subprocess.run(["./a.out"], input=stdin.encode('utf-8'), timeout=5, capture_output=True,
                ...
                output = subprocess.run(["node", fileloc], input=stdin.encode('utf-8'), timeout=5, capture_output=True,
                                        text=True)
            elif lang == "dart":
                output = subprocess.run(["dart", fileloc], input=stdin.encode('utf-8'), timeout=5, capture_output=True,
                                        text=True)
            else:
                return JsonResponse({'error': 'Language not supported.'}, status=403)

            # Cleanup compiled or temporary files
            remove(fileloc)
            ...
            error_message = str(e)
            error_lines = "\n".join(error_message.splitlines()[2:])
            return JsonResponse({'run_status': {'status': 'CE', 'output_html': error_lines.replace('\n', '<br>')}},
                                status=400)
...
def submit_code(request, pk):
    ...
    if request.is_ajax():
        ...
        BASE = Path(path.abspath(__file__)).parent.parent
        _filename = f'test_{uuid.uuid4()}.{lang}'
        ...
        for testCase in testCases:
            ...
            try:
                if lang == "PYTHON":
                    _output = subprocess.run(["python", _fileloc], input=data['input'].encode('utf-8'), timeout=question.time_limit, capture_output=True)
                elif lang == "cpp":
                    subprocess.call(["g++", _fileloc], timeout=question.time_limit)
                    _output = subprocess.run(["./a.out"], input=data['input'].encode('utf-8'), timeout=question.time_limit, capture_output=True)
                elif lang == "c":
                    subprocess.call(["gcc", _fileloc], timeout=question.time_limit)
                    _output = subprocess.run(["./a.out"], input=data['input'].encode('utf-8'), timeout=question.time_limit, capture_output=True)
                elif lang == "js":
                    _output = subprocess.run(["node", _fileloc], input=data['input'].encode('utf-8'), timeout=question.time_limit, capture_output=True)
                elif lang == "dart":
                    _output = subprocess.run(["dart", _fileloc], input=data['input'].encode('utf-8'), timeout=question.time_limit, capture_output=True)
                else:
                    return JsonResponse({'message': 'Language not supported.'}, status=403)

                output_ = _output.stdout.decode('utf-8')
                if _output.returncode != 0:
                    output_ = _output.stderr.decode('utf-8')
                    return JsonResponse({'message': 'Compilation Error', 'output': output_}, status=400)

                # Compare output with expected test case output
                ...
                if string_ == output_:
                    ...
                    return JsonResponse({'message': 'Test Cases Not Passed', 'output': output_}, status=400)

            except subprocess.TimeoutExpired:
                return JsonResponse({'message': 'Time Limit Exceeded'}, status=400)
            except Exception as e:
        ...
        if count == len(testCases):
            # Update score and solved status if all test cases passed
            if request.user != question.contest_of.curator.user:
                solved_check = SolvedOrNot.objects.filter(owner=Profile.objects.filter(user=request.user).first())
                if solved_check.exists():
                    solved_check[0].probs.add(question)
                else:
                    solved_or_not = SolvedOrNot(owner=Profile.objects.filter(user=request.user).first())
                    solved_or_not.save()
                    solved_check = SolvedOrNot.objects.filter(owner=Profile.objects.filter(user=request.user).first()).first()
                    solved_check.probs.add(question)

                scorecard_check = ScoreCard.objects.filter(_contest=question.contest_of, prof=Profile.objects.filter(user=request.user).first(), _ques=question)
                scoredcard_check_2 = ScoreCard.objects.filter(_contest=question.contest_of, prof=Profile.objects.filter(user=request.user).first())

                if not scorecard_check.exists() and scoredcard_check_2.exists():
                    scoredcard_check_2 = scoredcard_check_2.first()
                    endtime = str(question.contest_of.end_time)[:19]
                    if datetime.strptime(endtime, '%Y-%m-%d %H:%M:%S') > now:
                        scoredcard_check_2._ques.add(question)
                        scoredcard_check_2.score += question.score
                        scoredcard_check_2.time = now
                        scoredcard_check_2.save()

                        scorecards_for_total_points = ScoreCard.objects.filter(prof=Profile.objects.filter(user=request.user).first())
                        total = sum(int(scr.score) for scr in scorecards_for_total_points)
                        prof_of_the_user = Profile.objects.filter(user=request.user).first()
                        prof_of_the_user.total_points = total
                        prof_of_the_user.save()

            return JsonResponse({'message': 'Test Cases Passed', 'output': output_}, status=200)
...
def log_in(request):
    if request.method =="POST":
        ...
        if user is not None:
            login(request,user)
            # if user is learner then redirect to learner home
            prof_ =Profile.objects.filter(user =user)[0]
            if prof_._type =="LR":
                return redirect('/learner_home')
            else:
                return redirect('/curator_home')
...
def submit_code_w_c(request, pk):
    ...
    if request.is_ajax():
        code_ = request.POST.get('code_', '')
        lang = request.POST.get('lang', '')
        if not code_ or not lang:
            return JsonResponse({'message': 'Code or language missing'}, status=400)
        ...
        try:
            ...
            for testCase in testCases:
                ...
                try:
                    ...
                    if _output.returncode != 0:
                        output_ = _output.stderr.decode('utf-8')
                        logger.error(f"Compilation Error: {output_}")
                        return JsonResponse({'message': 'Compilation Error', 'output': output_}, status=400)
                    ...
                    if string_ == output_:
                        ...
                        logger.info(f"Test Cases Not Passed: Expected: {string_}, Got: {output_}")
                        return JsonResponse({'message': 'Test Cases Not Passed', 'Expected': string_, 'output': output_}, status=400)
